...
**The key is generated by the backend, not supplied by the caller.** An earlier version of this
signature took `key: str`, which contradicted `7-validation.md` VAL-023 — the key format is fixed at
`{application_id}/{uuid4}` — and `0-business-logic.md` DOC-003, which requires it to be opaque. A
caller that can choose the key can choose a path, and the whole point of VAL-023 is that no
client-controlled string ever reaches the filesystem. Corrected at T16.
