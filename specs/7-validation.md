...
| **VAL-027 step 1, "open the app cold"** | Read as `/calculator`. `T46` made `/` a public home page and moved the simulator; the claim being checked — a computed result on first paint with no interaction (`UX-009`) — is unchanged and belongs to that route. |
| **VAL-027 step 17, `fly apps restart`** | Run at T32 as `docker restart` against the production image with a mounted volume. The run was made *before* the deploy, deliberately (the deploy is what it gates), so there was no deployed app to restart. The claim is identical — session, rows and blobs outlive the container — and it is the same artefact. |
| **VAL-027 step 11, "change employment type"** | Could not be done at all: the wizard renders for `DRAFT` only, so after submission there was no way back to the answers the checklist is derived from. The API had always allowed the PATCH (`API-040`, narrowed at T21 for exactly this). Fixed at T32 by adding a **Your answers** editor beneath the checklist. |
